	...
	switch (action.type) {
		...
		case 'SET_VALUES':
			return {
				...
				errors: {
					...state.errors,
					...Object.keys(action.values).reduce((acc, key) => ({ ...acc, [key]: '' }), {}),
				},
			}
		case 'RESET':
...
export const useContactForm = () => {
	...
	const validateField = useCallback((field: FormField, value: string): string => {
		...
		const fieldNameMap: Record<FormField, string> = {
			...
			[FormField.WhoAreYou]: 'Who are you?',
			[FormField.ProjectLink]: 'Link to project',
			[FormField.TgHandle]: 'Tg handle',
	...
	const setValues = useCallback((newValues: Partial<Record<FormField, string>>) => {
