...
export const ModalManager: FC = memo((): JSX.Element => {
	...
	return (
		<>
			{state.contact && <ContactModal isOpen={state.contact} onClose={closeContactModal} />}

			{state.brandAssets && <BrandModal isOpen={state.brandAssets} onClose={closeBrandModal} />}

			{state.chains && <ChainsModal isOpen={state.chains} onClose={closeChainsModal} />}
		</>
	)
})
